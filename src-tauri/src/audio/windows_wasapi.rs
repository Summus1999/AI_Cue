...
use super::types::{AudioError, AudioFormat, AudioLevelEvent, CapturedAudio};
...
/// 波形采样点数（减少点数以便在 120px 宽度内更好地显示）
const WAVEFORM_POINTS: usize = 24;
...
/// 带事件发射的系统音频采集
pub fn capture_loopback_with_events(
    ready_tx: mpsc::Sender<Result<AudioFormat, AudioError>>,
    stop_rx: mpsc::Receiver<()>,
    ctx: &CaptureContext,
) -> Result<CapturedAudio, AudioError> {
...
/// 带事件发射的麦克风音频采集
pub fn capture_microphone_with_events(
    ready_tx: mpsc::Sender<Result<AudioFormat, AudioError>>,
    stop_rx: mpsc::Receiver<()>,
    ctx: &CaptureContext,
) -> Result<CapturedAudio, AudioError> {
...
fn capture_packets_with_events(
    ...
    let emit_interval = Duration::from_millis(EVENT_EMIT_INTERVAL_MS);
    ...
    loop {
        ...
        while packet_frames > 0 {
            ...
            if flags & AUDCLNT_BUFFERFLAGS_SILENT.0 as u32 != 0 {
                samples.extend(std::iter::repeat(0.0).take(sample_count));
                pending_samples.extend(std::iter::repeat(0.0).take(sample_count));
            } else {
                append_packet_samples(&mut samples, data_ptr, sample_count, source_format)?;
                append_packet_samples(&mut pending_samples, data_ptr, sample_count, source_format)?;
            }
