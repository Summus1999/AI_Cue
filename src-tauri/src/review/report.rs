...
pub fn build_review_report(db: &Database, session_id: &str) -> Result<ReviewReport, String> {
    ...
    let message_scores: Vec<MessageScore> =
        db_scores.iter().map(convert_db_message_score).collect();
...
fn get_session_info(db: &Database, session_id: &str) -> Result<SessionInfo, String> {
    ...
    if let Some(row) = rows.next().map_err(|e| e.to_string())? {
        ...
        let interview_context = match ctx_json {
            Some(json) if !json.is_empty() => serde_json::from_str::<InterviewContext>(&json).ok(),
            _ => None,
        };
...
fn convert_db_session_insight(
    db_insight: &database::SessionInsight,
