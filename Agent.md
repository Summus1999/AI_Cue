...
7. 再全量执行 `cargo test`
8. 只有两者都通过，才能执行 `git add`、`git commit`
9. 提交后必须将当前分支推送到对应远端分支
10. 只有验证通过且推送成功，才能把对应子任务从 `未完成` 改为 `已完成`
11. 只有对应子任务已标记完成，才能开始下一个子任务
12. 向用户汇报结果，并明确下一个 `未完成` 子任务是什么
