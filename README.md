...
- **自动抽取**：助手模式每轮回答后异步抽取面试题相关的结构化记忆
- **多来源融合**：支持助手对话自动抽取、用户显式指令、复盘手动录入三种来源
- **四层记忆**：情景记忆（具体问答事件）、语义记忆（稳定事实）、画像记忆（长期特征，由反思层归纳）、程序记忆（话术模板）
- **智能巩固**：向量相似度去重，避免同一事实重复记录，累加出现次数
- **反思与衰减**：active 记忆达阈值自动触发 LLM 反思生成画像特征；低重要性情景记忆按期衰减归档，显式指令与复盘录入豁免
- **检索注入**：relevance / recency / importance 三因子打分，assistant 模式默认启用，回答附带记忆来源引用
...
├── review/              # 面试复盘分析
├── logging.rs           # 结构化日志系统
├── memory.rs            # 个人记忆：抽取、巩固、反思、衰减生命周期
├── perf.rs              # 性能埋点
